<?php
	...
	use Workmark\Mailer\ParserException;
    ...
    class Parser
    {
        ...
        public function extractTags($mContent, $cReturnType = Parser::MERGED_UNIQUE, $bTrim = true)
        {
            ...
            if (!empty($mContent)) {
                if (is_string($mContent)) {
                    $aTags = array();
                    preg_match_all("/\{%\w+\%}/s", $mContent, $aTags);
                    if ($bTrim) {
        ...
        public function doParse($aTagValues, $mContent)
        {
            ...
            foreach ($aTagValues as $sTag => $sValue) {
                if (in_array($sTag, $aTags)) {
                    ...
                    if (is_string($mContent)) {
                        ...
                        foreach ($mContent as $mKey => &$mText) {
            ...
            if (!empty($aTags)) {
                $e = new ParserException('Some unparsed tags were detected in the content after parsing. ' . serialize($aTags));
                throw $e->setExtraInfo(
                    array(
                        'content'       => $mContent,
                        'unparsed_tags' => $aTags,
        ...
        protected function invalidCharactersCheck(&$sString)
        {
            ...
            foreach ($aInvalidCharacters as $sInvalidCharacter) {
                if (strpos($sString, $sInvalidCharacter) !== false) {
                	$string = substr($sString, strpos($sString, $sInvalidCharacter)-230, 270);
                    $e = new ParserException('Invalid characters detected in the content! :: "' . $sInvalidCharacter . '" near "' . $string . '"');
