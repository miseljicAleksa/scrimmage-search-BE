...
class UsersController  extends WorkmarkController
{
    ...
    public function editAction()
    {
    	...
    	if ($this->getRequest()->isPost())
    	{
    		...
    		unset($post['password']);
    		$post['id'] = $_GET['id'];
    		$users = $this->container->get(\Workmark\Model\Model::class)->get(\Workmark\Model\Users::class);
    		try{
    	...
    	$curUser = $this->container->get(\Workmark\User\User::class);
    	$user = clone $curUser;
    	$user->init($_GET['id']);
    	
    	return new ViewModel(['user'=>$user, 'status'=>$status]);
    ...
    public function listAction()
    {
    	...
    	$usersModel = $this->container->get(Model::class)->get(Users::class);
    	$totalUsers = (int)$usersModel->getTotal($REQUEST);
    	...
    	$users = $usersModel->getUsers($REQUEST);
    	...
    	$view = new ViewModel(['users'=>$users, 'pagination'=>$pagination]);
    	return $view;
    }
}
